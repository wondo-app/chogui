...
| --- | --- |
| [`chogui`](packages/choui) | The library: inline/runtime tag packs, DaisyUI components, the `StoryMarkup` renderer, the tag manifest + prop schema, and the precompiled/re-themeable CSS. Subpath exports: `.` · `./markup` · `./ui` · `./manifest` · `./props` · `./css` · `./preset`. |
| [`chogui-three`](packages/choui-three) | The framework-agnostic 3D device engine (three.js + cannon-es) that `choui` lazy-loads for the `dice`/`spintop`/`board` tags. Ships its runtime `assets/` (textures, sounds, models) and a `copy-assets.mjs` host-vendoring script. |

> The npm package names are `chogui` / `chogui-three`; the repo directories stay `packages/choui*`.
...
node node_modules/chogui-three/scripts/copy-assets.mjs   # → public/assets/choui-three/
```

`choui` ships precompiled CSS (`import "chogui/css"`) for zero-config hosts, and a
