...
All notable changes to `chogui-three` are documented here. The package is the
framework-agnostic 3D engine behind chogui's game-device tags, reached only through
...
- `scripts/copy-assets.mjs` — package-owned vendoring of the served runtime
  assets (`--dest <dir>`, default `public/assets/chogui-three/` under the CWD;
...
- `DiceDevice` now resolves its asset base through the engine's
  `resolveAssetPath()` (default `/assets/chogui-three/`) instead of a
