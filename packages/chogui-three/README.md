...
The three.js companion to **chogui** — a framework-agnostic, vanilla-TS 3D
...
The engine is reached only through a dynamic `import()` (in chogui's `use3DDevice`
...
- **Directly** — `new SpinTopDevice(props)` / `new BoardDevice(props)`, then
  `stage.add(device)`. This is what chogui's tags do.
...
the devices it uses. (This mirrors `chogui`, which also composes packs
...
(`/assets/chogui-three/`) → `CDN_ASSET_PATH` (jsDelivr) with `preferCdn`.
...
`publishable-story-tag-library` change), where chogui and chogui-three ship
