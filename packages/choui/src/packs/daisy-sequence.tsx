...
// `choui-timeline` namespace class so it can't clobber other `.timeline` use.
...
// Static, namespaced under `choui-timeline` so it can't affect other timelines.
const TIMELINE_STYLES = `.choui-timeline > li:first-child > hr:first-child{display:none}.choui-timeline > li:last-child > hr:last-child{display:none}`;
...
function Timeline({ direction = "vertical", children }: { direction?: string; children: ReactNode }) {
	...
	return <ul className={`timeline choui-timeline ${directionClass} my-4`}>{children}</ul>;
}
