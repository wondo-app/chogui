...
All notable changes to `choui` are documented here. This package
...
- **BREAKING for operator CSS**: package-shipped class prefixes renamed from
  `wondo-*` to `choui-*` ahead of the public publish — `choui-fx-<slug>`,
  `choui-timeline`, `choui-hover-gallery`. The prefix is public API for
  operator stylesheets; it is settled now so the published package never
  breaks it. Console warnings are prefixed `[choui]` for the same reason.
...
- `package.json` declares `license: MIT`, `files`, and `sideEffects: false`
  ahead of the coordinated publish (as `choui`, versioned and published
  together with `choui-three`).
...
interactive dice *function*: click to throw, the dice-box-threejs (WebGL) dice
...
- `roll` (void) — `[roll dice="2d6+3" var="atk"]`. Composes the `random` source
  ...
  *predetermined* roll via `@`-notation), and variable assignment via the host
...
Adds the reference *coordinating* tag-set (contrast the child-agnostic core
