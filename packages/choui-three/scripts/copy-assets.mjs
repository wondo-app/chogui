#!/usr/bin/env node
// Vendors choui-three's runtime assets (dice/board textures + hit/surface
// sounds) into a host's static dir so the engine can fetch them at runtime
// from `resolveAssetPath()` (default `/assets/choui-three/`):
...
const dest =
  destFlag !== -1 && process.argv[destFlag + 1]
    ...
    : join(process.cwd(), "public", "assets", "choui-three");

const src = join(__dirname, "..", "assets");

if (!existsSync(src)) {
  console.warn(`[choui-three copy-assets] source not found, skipping: ${src}`);
  process.exit(0);
}

mkdirSync(dest, { recursive: true });
cpSync(src, dest, { recursive: true });
console.log(`[choui-three copy-assets] ${src} → ${dest}`);
