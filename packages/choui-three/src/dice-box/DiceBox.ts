// @ts-nocheck — vendored JS, gradually adding types
...
class DiceBox {
	...
	constructor(sceneOrSelector, options = {}) {
		//private variables
		this.initialized = false
		// Accept either a THREE.Scene (from Stage) or a CSS selector string (legacy).
		this.container = null;
		if (typeof sceneOrSelector === 'string') {
			// SSR HAZARD: document.querySelector crashes in Node. This class must
			// only be instantiated client-side (the DiceRoller component dynamic-
			// imports it inside a `typeof window` guard).
			this.container = document.querySelector(sceneOrSelector);
			this.scene = new THREE.Scene();
			this.dimensions = new THREE.Vector2(this.container.clientWidth, this.container.clientHeight)
		} else {
			// External scene from Stage — no container, no own scene.
			this.scene = sceneOrSelector;
			// Allow caller to override tray dimensions (for coordinate scaling).
			const optDims = options.dimensions;
			this.dimensions = optDims
				? new THREE.Vector2(optDims.x, optDims.y)
				: new THREE.Vector2(280, 280);
		}
	...
	async initialize() {
		...
		if (this.container) {
			// Legacy mode: create own renderer + camera.
		...
		if (this.container) this.resizeWorld()
	...
	// Z-up coordinate system (upstream): floor is the XY plane, +Z is up.
	makeWorldBox(){
		...
		// Floor: cannon's default Plane faces +Z, so it's the XY ground plane as-is.
	...
	setDimensions(dimensions) {
		if (this.container) {
			...
			this.display.currentWidth = (dimensions?.x ?? 280) / 2;
			this.display.currentHeight = (dimensions?.y ?? 280) / 2;
		}
		if (dimensions) {
		...
		if (this.container) {
			// Legacy mode: create own camera + lights.
			...
			switch (this.animstate) {
				case 'selector':
					this.camera.position.y = this.selector.dice.length > 9 ? this.cameraHeight.far : (this.selector.dice.length < 6 ? this.cameraHeight.close : this.cameraHeight.medium);
					break;
				case 'throw': case 'afterthrow': default: this.camera.position.y = this.cameraHeight.far;
			}
	...
	getNotationVectors(notation, vector, boost, dist){
		...
		for (let i in notationVectors.set) {
			...
			for(let k = 0; k < numdice; k++){
				...
				// Z-up (upstream): horizontal = XY plane, vertical = Z.
				let pos = {
	...
	clearDice() {
		...
		setTimeout(() => { if (this.renderer) this.renderer.render(this.scene, this.camera); }, 100);
	}
	...
	// Reduced-motion / instant-result path: spawn dice, simulate physics to
	// completion, swap faces for predetermined results, sync meshes to final
	// body positions, and return the results — no animation.
	showResult(notation) {
		this.notationVectors = this.startClickThrow(notation);
		if (!this.notationVectors || this.notationVectors.error) return null;
		...
		// Sync meshes to final body positions.
		for (let i in this.scene.children) {
			let interact = this.scene.children[i];
			if (interact.body != undefined) {
				interact.position.copy(interact.body.position);
		...
		// Swap faces for predetermined results.
		if (this.notationVectors.result && this.notationVectors.result.length > 0) {
			for (let i = 0; i < this.notationVectors.result.length; i++) {
				let dicemesh = this.diceList[i];
				if (!dicemesh) continue;
