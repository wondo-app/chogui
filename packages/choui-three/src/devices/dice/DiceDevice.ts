// `[dice]` device — a polyhedral dice roller on the choui-three engine. It
...
export class DiceDevice implements Device<DiceResult> {
  ...
  mount(ctx: DeviceContext): void {
    ...
    this.initPromise = this.box.initialize().catch((err: unknown) => {
      if (!this.disposed) console.warn("[choui] DiceBox initialization failed:", err);
    });
  ...
  settle(): void {
    ...
    const roll = () => {
      ...
      this.box!.roll(notation).then((results: unknown) => {
        ...
        if (this.disposed) return;
        console.warn("[choui] Dice roll failed:", err);
        this.rolling = false;
        this.settled = true;
      });
    };
