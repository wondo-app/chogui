# Contributing a device to choui-three
...
import type { Device, DeviceContext, DeviceProps } from "choui-three";
...
```ts
import { Stage, registerDevice, createDevice } from "choui-three";
...
- **Assets.** Prefer bundler `?url` imports for assets you ship with the device
  ...
  default (`/assets/choui-three/`) → jsDelivr CDN fallback.
