...
> Publishes as **`chogui`** (with **`chogui-three`** as its 3D engine — the two
> version and publish together). The package name is `chogui`
...
- **`./preset` — re-themeable.** A Tailwind v4 fragment for consumers who run
  ...
  ```css
  @import "tailwindcss";
  @import "chogui/preset";
  @source "../node_modules/chogui/dist";
...
```ts
import { createStoryMarkup } from "chogui/markup";
...
Package-shipped selectors are namespaced under the neutral `chogui-` prefix
(`chogui-fx-*`, `chogui-timeline`, `chogui-hover-gallery`) — that prefix is public
API for host CSS.

## 3D device assets

The dice/spintop devices fetch textures and sounds at runtime from
`chogui-three`'s `resolveAssetPath()` (default `/assets/chogui-three/`). Either
vendor the assets — `node node_modules/chogui-three/scripts/copy-assets.mjs`, a
