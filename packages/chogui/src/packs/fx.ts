...
const fxStyles = `.chogui-fx-aside{font-style:italic;border-left:2px solid color-mix(in oklab, var(--color-primary) 45%, transparent);padding-left:1.25rem;opacity:0.85}`;
...
// injection), and we additionally restrict it to `chogui-fx-[a-z0-9-]+` so a
...
function fxClassName(value: unknown): string | undefined {
  ...
  const slug = value
    ...
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  return slug ? `chogui-fx-${slug}` : undefined;
}
