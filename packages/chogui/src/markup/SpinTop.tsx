// `[spintop]` — an N-sided spinning top on the chogui-three engine, driven by a cannon-es
...
export function SpinTop(props: SpinTopProps) {
  ...
  useEffect(() => {
    ...
    if (!props.flavor) {
      console.warn(
        `[chogui] [spintop var="${props.varName}"] self-roll is flavor-only — it must not gate a branch. Set flavor to silence this.`,
      );
    }
